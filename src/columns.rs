...
fn col_argocd_revision<'a>(ctx: &CellContext<'a>) -> Cow<'a, str> {
    // Shares `argocd::revision` with the view: the fields it reads grew a
    // multi-source form, and a second copy here missed it.
    let rev = crate::argocd::revision(ctx.obj);
...
mod tests {
    ...
    fn argocd_application_cells_read_sync_health_and_revision() {
        ...
        assert_eq!(at("AUTO-SYNC"), "on");
        assert_eq!(status_idx, headers.iter().position(|x| *x == "SYNC"));
    }

    /// Most Applications are multi-source, and those record revisions under the
    /// plural field, so the column has to read it or it sits empty.
    #[test]
    fn argocd_revision_reads_the_multi_source_field() {
        let a = obj(json!({
            "apiVersion": "argoproj.io/v1alpha1", "kind": "Application",
            "metadata": {"name": "a"},
